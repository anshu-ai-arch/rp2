...
def run_alpha07_fold1_verification():
    device = get_m1_device()
    print("\n=================================================================")
    print("      ECG VERIFICATION RUN: FOLD 1 WITH SAMPLING ALPHA = 0.70    ")
    print("=================================================================")
    ...
    # 2. Per-class sampling weights for alpha = 0.70
    alpha = 0.70
    ...
    print(f"\n[*] Per-Class Sampling Weights (alpha = 0.70, w_4 = 0.0):")
    for c in range(5):
    ...
    print(f"\n[*] Theoretical Expected Sampling Proportions (alpha = 0.70):")
    for c in range(5):
    ...
    checkpoint_path = checkpoint_dir / "ecg_kfold_alpha07_fold1_20ep_verification_best.pth"
    ...
    # Save results JSON
    results_json = Path("experiments/results_ecg_kfold_alpha07_fold1_20ep_verification.json")
    save_summary = {
        "experiment": "Fold 1 Verification (alpha=0.70, 20 epochs)",
        "hyperparameters": {
            "alpha": 0.70,
            "lr": 0.002018,
            "weight_decay": 2.35e-5,
...
if __name__ == "__main__":
    run_alpha07_fold1_verification()

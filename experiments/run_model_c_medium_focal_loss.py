...
class MultiClassFocalLoss(nn.Module):
    """
    Mathematically rigorous Multi-Class Focal Loss operating on raw model logits.
...
def run_experiment_focal_loss(num_epochs: int = 20):
    device = get_m1_device()
    print("\n=================================================================")
    print("      EXPERIMENT B — MEDIUM CNN WITH FOCAL LOSS (gamma=2.0)       ")
    print("=================================================================")
    ...
    print(f"[*] Loss Function: MultiClassFocalLoss(gamma=2.0, weight=None, reduction='mean')\n")
    ...
    # ONLY CHANGE: Replace nn.CrossEntropyLoss() with MultiClassFocalLoss(gamma=2.0, weight=None, reduction='mean')
    criterion = MultiClassFocalLoss(gamma=2.0, weight=None, reduction='mean')
    optimizer = optim.Adam(model.parameters(), lr=0.002018, weight_decay=2.35e-5)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", factor=0.5, patience=3)

    best_val_acc = 0.0
    checkpoint_path = Path("checkpoints/ecg_focal_loss_best.pth")
            ...
            torch.save(
                {
                    ...
                    "cnn_channels": [64, 128, 128],
                    "loss_function": "MultiClassFocalLoss(gamma=2.0, weight=None, reduction='mean')"
                },
                checkpoint_path
            )

    print(f"\n[✓] Focal Loss Experiment Training Complete! Best Val Acc: {best_val_acc:.2f}%")
    ...
    results_file = Path("experiments/results_ecg_focal_loss.json")
    save_summary = {
        "model_name": "Experiment B (Medium CNN + Focal Loss gamma=2.0)",
        "trainable_params": trainable_params,
    ...
    print(f"[✓] Saved Focal Loss evaluation results to '{results_file}'")
    return save_summary


if __name__ == "__main__":
    run_experiment_focal_loss(num_epochs=20)
